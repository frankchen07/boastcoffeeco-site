...
export interface ShopifyClient {
  getProducts(first?: number): Promise<NormalizedProduct[]>;
  getProduct(handle: string): Promise<NormalizedProduct | null>;
  createCart(): Promise<NormalizedCart>;
  addToCart(
    cartId: string,
    lines: { merchandiseId: string; quantity: number; sellingPlanId?: string }[]
  ): Promise<NormalizedCart>;
  removeFromCart(cartId: string, lineIds: string[]): Promise<NormalizedCart>;
  updateCartLine(cartId: string, lineId: string, quantity: number): Promise<NormalizedCart>;
  getCart(cartId: string): Promise<NormalizedCart | null>;
}
...
export function createShopifyClient({
  ...
  /** Pin queries to a Market via @inContext(country). Omit to use the shop's default market. */
  country?: string;
}): ShopifyClient {
  const endpoint = storeDomain ? `https://${storeDomain}/api/2026-04/graphql.json` : null;
  ...
  async function shopifyFetch<T>(
    ...
    if (!endpoint || !storefrontToken) {
      throw new Error(
        "Shopify credentials not configured. Set the store domain and storefront access token in .env"
      );
    }
  ...
  async function getProducts(first = 12): Promise<NormalizedProduct[]> {
  ...
  async function getProduct(handle: string): Promise<NormalizedProduct | null> {
  ...
  async function createCart(): Promise<NormalizedCart> {
  ...
  async function removeFromCart(cartId: string, lineIds: string[]): Promise<NormalizedCart> {
  ...
  async function getCart(cartId: string): Promise<NormalizedCart | null> {
