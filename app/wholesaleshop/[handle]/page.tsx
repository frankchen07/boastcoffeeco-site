...
import { getProduct } from "@/lib/shopify-wholesale";
import { formatPrice, MOCK_PRODUCTS } from "@/lib/shopify-client";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (!product) return { title: "Product not found", robots };
    return {
      title: product.title,
    ...
    return mock
      ? { title: mock.title, description: mock.description, robots }
      ...
      <div className="container-md py-16">
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <AddToCartButton product={product} useCartHook={useWholesaleCart} />
          </div>
        </div>
      </div>
    </div>
